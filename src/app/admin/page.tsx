      ...
      <div className="grid md:grid-cols-3 gap-6">
            ...
            {[
              ...
              <li
                ...
                className={`flex flex-wrap md:grid md:grid-cols-12 gap-3 md:gap-4 items-center p-4 rounded-xl border transition ${
                  s.stav === "ted"
                    ? "bg-[#c8a867]/10 border-[#c8a867]/40"
                    : "bg-white/50 border-[#c8a867]/10"
                }`}
              >
                <div className="md:col-span-2 flex items-baseline gap-2 md:block">
                  <p className="display text-2xl">{s.cas}</p>
                  <p className="text-xs text-[#8e6f3f]">{s.trv}</p>
                </div>
                <div className="md:col-span-7 flex-1 min-w-0">
                  <p className="font-medium">{s.klient}</p>
                  <p className="text-xs text-[#5a4a32] mt-0.5">{s.typ}</p>
                </div>
                <div className="md:col-span-3 text-right">
                  {s.stav === "ted" ? (
                    <span className="text-[10px] uppercase tracking-widest text-[#8e6f3f]">
                      Za chvíli
                    </span>
                  ) : (
                    <Link
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white rounded-2xl p-8 shadow-md shadow-[#c8a867]/5">
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-xl bg-[#c8a867]/10 border border-[#c8a867]/30">
              <p className="text-xs uppercase tracking-widest text-[#8e6f3f] mb-1">
                Termín · DPH
              </p>
              <p className="font-medium mb-2">4 klienti · 25. května</p>
              <p className="text-sm text-[#5a4a32] mb-3">
                Acme, Beta Tech, Gamma, Delta — kontrolní hlášení připravit
                a poslat ke schválení.
              </p>
              <Link
                href="#"
                className="text-xs text-[#8e6f3f] hover:text-[#2a1f12] underline underline-offset-4"
              >
                Otevřít →
              </Link>
            </div>

            <div className="p-4 rounded-xl bg-white/50 border border-[#c8a867]/10">
              <p className="text-xs uppercase tracking-widest text-[#8e6f3f] mb-1">
                Faktury · vystavit
              </p>
